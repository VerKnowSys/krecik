...
// #[async_main] // <- starts the system and block until future resolves
// async fn main() {
...
impl Handler<Checks> for CurlActor {
...
async fn main() {
    ...
    let generic_curl_actor = SyncArbiter::start(4, || CurlActor);
    // let pongo_curl_actor = SyncArbiter::start(4, || CurlActor);

    match generic_curl_actor.send(Checks(api::all_checks())).await {
        Ok(val) => debug!("{:?}", val),
        Err(_) => (),
    };
